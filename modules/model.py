...
class CRNN(nn.Module):
    ...
    def __init__(self, num_chars, rnn_input_size=256, rnn_hidden_size=256, backbone="resnet18"):
        ...
        if backbone == "resnet18":
            ...
            self.outfeature = 1024

        elif backbone == "resnet101":
            model = models.resnet101(pretrained=True)
            self.outfeature = 1024
            
        model_modules = list(model.children())[:-3]
        self.cnn_p1 = nn.Sequential(*model_modules)
        
        # CNN Part 2
        self.cnn_p2 = nn.Sequential(
            nn.Conv2d(self.outfeature, self.outfeature, kernel_size=(3,6), stride=1, padding=1),
        ...
        self.SequenceModel = nn.Sequential(
            nn.Linear(4 * self.outfeature, self.rnn_input_size),
            BidirectionalLSTM(self.rnn_input_size, self.rnn_hidden_size, self.rnn_hidden_size),
            BidirectionalLSTM(self.rnn_hidden_size, self.rnn_hidden_size, self.rnn_hidden_size),
            nn.Linear(self.rnn_hidden_size, num_chars)
            )
    ...
    def forward(self, batch):
        ...
        T = batch.size(1)
        batch = batch.view(batch_size, T, 4 * self.outfeature) # [batch_size, T==width, num_features==channels*height]
        # print(batch.size()) # torch.Size([-1, 10, 1024])

        batch = self.SequenceModel(batch)# torch.Size([-1, 10, 20])
